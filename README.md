...
Второе число - общий лимит теста в секундах, а не время на каждый адрес. Например, `/pooltest zec 300 10` вернёт результат примерно через 5 минут. Тест измеряет только успешность TCP-подключения и задержку. Настройки пулов на ASIC автоматически не меняются.
