...
return view.extend({
	...
	render: function(res) {
		...
		var settingsForm = E('form', { 'class': 'cbi-section', id: 'asic-settings' }, [
			...
			field('Telegram IP attempts', input('telegram_max_candidates', cfg.telegram_max_candidates || '2')),
			field('Telegram send retries', input('telegram_send_retries', cfg.telegram_send_retries || '2')),
			field('Telegram probe interval, sec', input('telegram_probe_interval', cfg.telegram_probe_interval || '300')),
			field('Telegram самотест, сек', input('telegram_heartbeat_interval', cfg.telegram_heartbeat_interval || '0')),
			field('Debug log', select('debug_log', cfg.debug_log || '1', [ opt('1', 'On'), opt('0', 'Off') ])),
			field('Тест пулов, сек на адрес', input('pool_test_duration', cfg.pool_test_duration || '300')),
			field('Тест пулов, интервал сек', input('pool_test_interval', cfg.pool_test_interval || '10')),
			field('Тест пулов, timeout сек', input('pool_test_timeout', cfg.pool_test_timeout || '5')),
